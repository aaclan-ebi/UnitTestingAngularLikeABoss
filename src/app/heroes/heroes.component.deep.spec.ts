...
describe('HeroDetailComponent (deep tests)', () => {
  ...
  it(`should call heroservice.deleteHero when the HeroComponent's delete button is clicked`, () => {
    ...
    // this method will raise ANY event you put in here. If there is no listener in the template (eventname)="blah" then nothing will
    // listen. so if we raise "crap" then there must be (crap)="blah()" in the parent template. That's all that matter. So technically
